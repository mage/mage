...
var supportedTypes = ['.yaml', '.json', '.json5', '.js'];
...
// This will be overriden by MAGE once during its instantiation, but
...
class ConfigurationError extends Error {
	constructor(message, filename, error) {
		super(message);
 ...
 *
 * @param {String} name       The key to assign the value to.
 ...
 * @param {*} value      Value
 */
exports.set = function (trail, value) {
	const obj = {};
	const subpaths = parseTrail(trail);
...
exports.initialize = function () {
	...
	var customConf = loadConfig(configDir, 'custom', true, true, false);
	var customConfig = new Matryoshka(customConf.config, customConf.source);
...
exports.expose = function (mageInstance) {
	const httpServer = mageInstance.core.httpServer;
	...
	function handler(req, res, path, query) {
		...
		const config = mageInstance.getConfig(query.appName);
		res.writeHead(200, {
	...
	httpServer.addRoute(new RegExp('^/config'), handler, 'simple');
};
