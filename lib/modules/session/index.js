...
mage.core.msgServer.msgStream.addAddressType('session', {
	extendInterval: sessionTTL / 2 * 1000,
	extend: function (address, host, cb) {
		try {
			const fullSessionKey = parseAddress(address).fullSessionKey;
			this.lookup(fullSessionKey, host, function () {
				cb();
			});
		} catch (error) {
			logger.warning('Error parsing session key:', error);
			return cb();
		}
	},
