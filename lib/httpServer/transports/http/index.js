...
/**
 * The RequestError class can be used to emit or otherwise
 * throw request-related errors.
 */
class RequestError extends Error {
	constructor(code, message, req) {
		...
		this.remoteAddress = forwarded(req, req.headers);
		this.url = req.url;
...
exports.initialize = function (mageLogger, cfg) {
	...
	if (cfg.maxRequestSize) {
		maxRequestSize = cfg.maxRequestSize * 1024;
	}
 ...
 *
 * This function currently only takes care of
 * checking the HTTP POST size.
 *
 * @param {*} req HTTP request
 */
function setRequestFilters(req, cb) {
	var realOn = req.on.bind(req);
	var byteSize = 0;

	function isRequestTooLarge() {
		return byteSize > maxRequestSize;
	}
	...
	.on('data', function (chunk) {
		...
		if (isRequestTooLarge()) {
			...
			return cb(new RequestError(413, 'Entity too large', req), req);
		}
	})
	.on('end', () => {
		cb(null, req);
	...
	// Here, we would normally want to use streams, but unfortunately streams
	// do not wrap or proxy methods from upstream objects; therefore, we need to
	// return the request object as-is, but still provide a mechanism to interrupt
	// that allows us to interrupt the emission of data event in the case
	// where the request was already found to be too large.
	//
	// The other reason we need this is because emitting an error event from within
	// an event callback will not interrupt the current event emission; for instance, if
	// you have two callbacks listening on the 'data' event and the first one emits an error,
	// the second callback will still get emitted. removeAllListeners also do not have
	// any effect in this case.
	req.on = function (eventName, func) {
		if (eventName !== 'data') {
			return realOn(eventName, func);
		}

		realOn('data', function (chunk) {
			if (!isRequestTooLarge()) {
				func(chunk);
			}
		});
...
function requestHandler(req, res) {
	...
	if (route && route.type === 'proxy') {
		...
		return route.handler(req, urlInfo);
	}
