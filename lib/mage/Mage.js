...
Mage.prototype.setupCoreLibs = function () {
	...
	// Add logger default config here and not in loggingService module
	// otherwise there is a circular dependency
	core.config.setTopLevelDefault('logging', path.join(__dirname, '../loggingService/config.yaml'));
...
Mage.prototype.getConfig = function (appName) {
	...
	return {
		...
		server: {
			msgStream: this.core.msgServer.getPublicConfig(),
			commandCenter: app.commandCenter.getPublicConfig(),
			savvy: this.core.savvy.getPublicConfig()
		},
