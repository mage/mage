...
// Vault backends registered by the end user

var registeredVaultBackends = {};
...
exports.registerVaultBackend = function (type, mod) {
	registeredVaultBackends[type] = mod;
};

function getVaultMod(type) {
	var backend = registeredVaultBackends[type];

	if (!backend) {
		backend = registeredVaultBackends[type] = require('./vaults/' + type);
	}
