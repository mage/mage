...
FileVault.prototype.setup = function (cfg, cb) {
	...
	this.filePath = this.path = path.resolve(cfg.path || './filevault');
...
FileVault.prototype.open = function (cb) {
	...
	fs.stat(this.filePath, function (err, stats) {
		...
		if (!stats.isDirectory()) {
			logger.emergency('Path', this.filePath, 'is not a directory. Please check your vault configuration.');
