...
MsgStream.prototype.initScheduler = function () {
	const conf = addressTypes.session;

	if (!conf) {
		logger.warning('Cannot find session config');
		return;
	}

	const extendInterval = conf.extendInterval;

	if (!extendInterval) {
		logger.warning('Extend interval not set');
		return;
	}

	this.scheduler = new Scheduler(extendInterval, 5, (elems) => {
		const promises = elems.map((elem) => {
			return new Promise((resolve, reject) => {
				const address = elem.address;
				const host = elem.host;

				const configuration = addressTypes.session;
				configuration.extend(address, host, function (err) {
					if (err) {
						return reject(err);
					}
...
MsgStream.prototype.getAddressesOfType = function (type) {
	const addressMap = this.addressMap;
	const addresses = [];

	Object
		.keys(this.addressMap)
		.forEach(function (address) {
			const host = addressMap[address];

			if (host.address.type === type) {
				addresses.push({ address: address, host: host });
			}
		});
