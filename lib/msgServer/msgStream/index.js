...
function MsgStream(cfg) {
	...
	for (const apiInfo of Object.entries(addressTypes)) {
		const type = apiInfo[0];
		const configuration = apiInfo[1];
		const extendInterval = configuration.extendInterval;
		...
		if (extendInterval) {
			if (!configuration.extend) {
				logger.warning
					...
					.log('Extend interval set but no extend function defined');

				continue;
			}

			const interval = setInterval(function () {
				async.each(that.getAddressesOfType(type), function (entry, cb) {
					configuration.extend(entry.address, entry.host, cb);
				}, function (error) {
					if (error) {
						logger.error(error);
					}
